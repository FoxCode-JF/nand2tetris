...
//	parse command name - used later for determining current command type
...
{
	if(this->currentCommand.empty()) this->currentCmdType = cmdType::NO_COMMAND;
	else
	{
		std::istringstream ss(this->currentCommand);
		ss >> this->arg0;
		this->currentCmdType = s_command[this->arg0];
	}
